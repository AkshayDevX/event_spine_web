      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-primary/20 to-cyan/10 border border-white/10 p-8 flex items-center justify-between">
        ...
        <div className="relative z-10">
          ...
          <p className="text-foreground/70 max-w-lg">
            Your workspace is running smoothly. You have 12 active workflows
            processing events across 4 integrations.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {kpis.map((kpi) => {
          ...
          return (
            <Card
              key={kpi.title}
              className="bg-white/[0.03] border-white/5 backdrop-blur-xl shadow-lg hover:border-white/10 transition-colors"
            >
              <Card.Content className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div
                    className={`p-3 rounded-2xl bg-white/5 border border-white/5 ${kpi.color}`}
                  >
        ...
        <Card className="lg:col-span-2 bg-white/[0.02] border-white/5 backdrop-blur-xl">
          <Card.Header className="px-6 py-5 border-b border-white/5 flex justify-between items-center">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="text-cyan hover:bg-cyan/10 flex items-center gap-1"
            >
              View All
          ...
          <Card.Content className="p-0">
            <div className="divide-y divide-white/5">
              {recentActivity.map((activity) => (
                <div
                  key={activity.id}
                  className="p-4 px-6 flex items-center justify-between hover:bg-white/[0.02] transition-colors cursor-pointer group"
                >
                  <div className="flex items-center gap-4">
                    <div className="p-2 rounded-full bg-white/5">
                      ...
                      {activity.status === "running" && (
                        <Clock className="h-4 w-4 text-cyan animate-pulse" />
                      )}
                    </div>
                    <div>
                      <h4 className="text-sm font-medium text-white group-hover:text-cyan transition-colors">
                        {activity.name}
                      </h4>
          ...
          <Card className="bg-gradient-to-br from-primary/10 to-transparent border-primary/20 backdrop-blur-xl relative overflow-hidden group cursor-pointer">
            ...
            <Card.Content className="p-6 relative z-10">
              ...
              <p className="text-sm text-foreground/70 mb-4">
                Unlock unlimited workflows, custom domains, and priority
                support.
              </p>
              <Button
                size="sm"
                className="w-full bg-primary/20 text-primary border border-primary/30 hover:bg-primary hover:text-white transition-all"
              >
                View Plans
              </Button>
            </Card.Content>
